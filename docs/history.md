...
## 2026-06-07 - 落地 imagegen 首页视觉到小程序

任务：参考 `$imagegen` 生成的首页效果图，重新优化微信小程序首页界面。

改动项：

- 首页 Hero 改为整张大图承载主视觉，标题、副标题和剩余次数胶囊直接叠加在图片上。
- 新增三步流程卡片：上传照片、选择风格、AI 生成。
- 风格卡片改为更接近参考图的横向短卡比例，补充 HOT 标签、底部渐变遮罩和右下角圆形箭头。
- 页面背景、间距、圆角、阴影和固定底部 CTA 统一向参考图的暖色写真馆风格收敛。
- 保留原有风格选择、跳转上传、查看全部和底部导航交互逻辑。

验证：

- `node --check frontend/weapp/pages/home/index.js` 通过。
- 小程序 `app.json` 和首页 `index.json` 解析通过。
